...
# Exact nav the user wants: Shop, New Arrivals, Best Sellers, Collections (mega),
# Bundles, Track My Order, Login. (url, name, sequence, is_mega)
DESIRED_MENUS = [
    ("/shop", "Shop", 10, False),
    ("/shop?order=create_date desc", "New Arrivals", 20, False),
    ("/shop", "Best Sellers", 30, False),
    ("#collections", "Collections", 40, True),   # mega menu
...
class Website(models.AbstractModel):
    ...
    def _ynf_setup_menus(self):
        ...
        # Remove all existing child menus under the top menu (clean slate)
        existing = Menu.search([("parent_id", "=", parent_id)])
        existing.unlink()

        # Create the desired menus
